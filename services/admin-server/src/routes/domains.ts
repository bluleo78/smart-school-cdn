...
export async function domainRoutes(
  ...
  app.put<{
    ...
    if (stale_if_error_secs !== undefined && stale_if_error_secs !== null) {
      if (!Number.isInteger(stale_if_error_secs) || stale_if_error_secs < 0 || stale_if_error_secs > 7 * 86400) {
        return reply.status(400).send({
          error: 'invalid_input',
          message: 'stale_if_error_secs는 0~604800(7일) 범위의 정수이거나 null이어야 합니다.',
        });
      }
    }
