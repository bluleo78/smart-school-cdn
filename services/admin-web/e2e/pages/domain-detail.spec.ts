...
test.describe('도메인 상세 — DomainStackedChart Tooltip 포맷 (#86)', () => {
  ...
  /**
   * 이슈 #236 회귀 방지 — Recharts Tooltip formatter는 dataKey의 "원본 카운트"를 받는데
   * `Math.round(v * 100)%` 식을 그대로 쓰면 1698 → 169800% 같은 거짓값이 노출되던 버그.
   * 수정 후: 같은 시점 4시리즈 합 대비 비율(%)로 환산해야 한다. 각 값은 0~100 범위 안에 있어야 한다.
   */
  test('스택 차트 hover tooltip이 0~100% 범위의 정규화된 비율로 표시된다 (회귀: #236)', async ({ page }) => {
    await setupDetailMocks(page);
    await page.goto('/domains/textbook.com');
    ...
    // 버그 상태: 91500%, 169800% 처럼 100을 훨씬 초과하는 값이 노출됨.
...
test.describe('도메인 상세 — 브라우저 탭 제목 (#78)', () => {
   ...
   * 수정 후: "textbook.com — 도메인 | Smart School CDN" 형태로 호스트명이 포함되어야 한다.
   */
  test('도메인 상세 페이지 title에 호스트명이 포함된다', async ({ page }) => {
    ...
    await expect(page).toHaveTitle('textbook.com — 도메인 | Smart School CDN');
  });

  test('도메인 상세 → 목록 복귀 시 title이 "도메인 | Smart School CDN"으로 복원된다', async ({ page }) => {
    // 언마운트 cleanup: return () => { document.title = '도메인 | Smart School CDN'; }
    await setupDetailMocks(page);
    await mockApi(page, 'GET', '/domains', []);
    await page.goto('/domains/textbook.com');
    ...
    // 목록 페이지로 돌아왔을 때 AppLayout이 title을 "도메인 | Smart School CDN"으로 복원해야 한다
    await expect(page).toHaveTitle('도메인 | Smart School CDN');
  });
});
